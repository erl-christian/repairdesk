...
export default function RepairRequestDetailsModal({
  repairId,
  isOpen,
  onClose,
}: Props) {
  ...
  // NEW: Fetch timeline for this specific repair request
  const {
    data: timeline,
    isFetching: isTimelineFetching,
    isError: isTimelineError,
  } = useRepairTimeline(request?.id ?? "");
          ...
          {request && !isFetching && (
            <p className="text-center text-sm text-muted-foreground mt-1">
              ...
              <span className="font-medium text-foreground">
                {request.publicTicketNumber ?? "N/A"}
              </span>{" "}
              • Status:{" "}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      ...
                      <p className="text-sm font-medium capitalize">
                        {request.serviceMethod?.replaceAll("_", " ") ??
                          "Unknown"}
                      </p>
                    </div>
